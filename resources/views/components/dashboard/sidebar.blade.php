...
<div class="w-full h-full bg-white flex flex-col transition-all duration-300">
    ...
    <div class="px-[8px] mb-[16px]">
        ...
        <div class="space-y-1">
            ...
            @if($isOwner)
            <a href="{{ route('business_owner.agents.index') }}"
               class="flex items-center rounded-[4px] p-2 transition-all duration-300 {{ request()->routeIs('business_owner.agents.index') ? 'bg-[#f6f6f5] text-[#1e1d1d]' : 'hover:bg-gray-50 text-[#1e1d1d]' }}"
               :class="sidebarOpen ? 'space-x-[10px]' : 'justify-center'">
