...
<div x-data="{ open: false, showButton: false }"
     x-init="
        $nextTick(() => {
            if ($refs.descriptionText.scrollHeight > 72) {
                showButton = true;
            }
        });
     ">
    <h3 class="text-[18px] font-medium text-black tracking-[-0.36px] leading-[1.28]">About this apartment</h3>
    <div x-ref="descriptionText"
         class="mt-[20px] text-[16px] text-[#282828] leading-[1.5] transition-all duration-300"
         :class="{ 'max-h-[72px] overflow-hidden': !open && showButton }"
