...
<div class="border-b border-gray-200 py-[20px] px-[16px]">
    <button @click="activeAccordion = activeAccordion === {{ $id }} ? null : {{ $id }}"
        ...
        <img src="{{ asset('images/chevron.svg') }}" alt="Dropdown Arrow"
             class="w-[32px] h-[32px] transition-transform duration-200"
             :class="activeAccordion === {{ $id }} ? 'rotate-180' : ''">    </button>
    <div x-show="activeAccordion === {{ $id }}" x-collapse class="pb-[12px] pt-[16px] text-[#464646] leading-[1.5] text-[16px]">
        {{ $slot }}
    </div>
</div>
