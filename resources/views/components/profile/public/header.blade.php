@props(['user'])

<div class="flex items-center gap-[24px] relative">
    {{-- Profile Photo --}}
    <div class="w-[126px] h-[126px] rounded-full border-2 border-light-gray overflow-hidden flex-shrink-0">
        <img src="{{ $user->getProfilePhotoUrl() }}" alt="{{ $user->name }}" class="w-full h-full object-cover">
    </div>

    {{-- Info --}}
    <div class="flex flex-col">
        <div class="flex items-center gap-[2px]">
            <h1 class="text-[22px] font-medium text-[#1e1d1d] tracking-[-0.44px] leading-[1.28]">{{ $user->name }}</h1>
            <img src="{{ asset('images/verified_user.svg') }}" alt="Verified" class="w-[23px] h-[23px]">
        </div>
        <p class="text-[14px] text-[#464646]">{{ $user->display_role }} • License no. {{ $user->license_number }}</p>
    </div>

    {{-- Flag/Report --}}
    <button class="absolute top-0 right-0">
        <img src="{{ asset('images/blue_flag.svg') }}" alt="Report" class="w-[25px] h-[25px]">
    </button>
</div>
