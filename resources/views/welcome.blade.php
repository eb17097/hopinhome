                        ...
                        <div class="relative" :style="openFilter === 'propertyType' ? 'filter: drop-shadow(0 4px 12px rgba(0, 0, 0, 0.06)) drop-shadow(0 2px 4px rgba(0,0,0,0.02))' : ''">
                            ...
                            <template x-if="openFilter === 'propertyType'">
                                <div class="absolute top-0 left-0 w-full">
                                    ...
                                    <div class="absolute z-10 top-[60px] left-0 bg-white border border-[#E8E8E7] rounded-b-[12px] rounded-tr-[12px] p-4 grid grid-cols-6 gap-3 w-[832px]" @click.away="openFilter = null">
                        ...
                        <div class="relative" :style="openFilter === 'bedrooms' ? 'filter: drop-shadow(0 4px 12px rgba(0, 0, 0, 0.06)) drop-shadow(0 2px 4px rgba(0,0,0,0.02))' : ''">
                            ...
                            <template x-if="openFilter === 'bedrooms'">
                                <div class="absolute top-0 left-0 w-full">
                                    ...
                                    <div class="absolute z-10 top-[60px] left-0 bg-white border border-[#E8E8E7] rounded-b-[10px] rounded-tr-[10px] p-4 pt-5 flex items-center gap-[12px] w-max" @click.away="openFilter = null">
                                        @foreach(['Studio', '1', '2', '3', '4', '5+'] as $val)
                                            <button type="button" @click="toggleBedroom('{{ $val }}')" class="flex items-center justify-center transition-all duration-150 text-[16px] font-medium focus:outline-none {{ $val === 'Studio' ? 'px-5 py-[7px]' : 'w-[40px] h-[40px]' }} rounded-full" :class="selectedBedrooms.includes('{{ $val }}') ? 'bg-[#1447D4] text-white shadow-sm' : 'bg-white border border-[#E2E2E2] text-[#222222] hover:border-[#222222] hover:bg-gray-50'">{{ $val }}</button>
                                        @endforeach
                                    </div>
                                </div>
                            </template>
                                    ...
                                    <div class="absolute z-10 top-[60px] left-0 bg-white border border-[#E8E8E7] rounded-b-[10px] rounded-tr-[10px] p-6 w-[440px]" @click.away="openFilter = null">
                                        <div class="flex gap-4 mb-6">
                                            <div class="flex-1">
                                                ...
                                                <div class="relative">
                                                    <input type="number" x-model.number="minPrice" min="0" @input="if(minPrice > (maxPrice || maxRange)) minPrice = (maxPrice || maxRange)" class="w-full border border-[#E8E8E7] rounded-[8px] px-4 py-3 text-[15px] font-medium text-[#1E1D1D] focus:ring-0 focus:border-[#1447D4] [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none">
                                            ...
                                            <div class="flex-1">
                                                ...
                                                <div class="relative">
                                                    <input type="number" x-model.number="maxPrice" min="0" @input="if(maxPrice < minPrice) maxPrice = minPrice" class="w-full border border-[#E8E8E7] rounded-[8px] px-4 py-3 text-[15px] font-medium text-[#1E1D1D] focus:ring-0 focus:border-[#1447D4] [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" placeholder="Any">
                                                    <span class="absolute right-4 top-1/2 -translate-y-1/2 text-[#707070] text-[14px]">AED</span>
                                                </div>
                                            </div>
                                        </div>
            ...
            @foreach([['name'=>'Dubai','image'=>'https://images.unsplash.com/photo-1546412414-e1885259563a?q=80&w=800&auto=format&fit=crop'],['name'=>'Abu Dhabi','image'=>'https://images.unsplash.com/photo-1583997052301-0042b33fc598?q=80&w=800&auto=format&fit=crop'],['name'=>'Sharjah','image'=>'https://images.unsplash.com/photo-1578895210405-907db486c111?q=80&w=800&auto=format&fit=crop'],['name'=>'Al Ain','image'=>'https://images.unsplash.com/photo-1546412414-e1885259563a?q=80&w=800&auto=format&fit=crop']] as $city)
                ...
                <a href="{{ route('listings.search', ['location' => $citySlug]) }}" class="group block">
                    <div class="overflow-hidden rounded-[6px] aspect-[277/172] mb-3">
                        <img src="{{ $city['image'] }}" alt="{{ $city['name'] }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                    </div>
...
<div class="bg-white py-[40px]">
    <div class="max-w-[1204px] mx-auto px-4 lg:px-0">
        <h2 class="text-[32px] font-medium leading-[1.28] tracking-[-0.64px] text-[#1E1D1D] mb-8 font-['General_Sans',_sans-serif]">Browse by property type</h2>
...
<div class="bg-[#F9F9F8] py-[96px]">
    <div class="max-w-[1204px] mx-auto px-4 lg:px-0">
        <div class="flex flex-col md:flex-row md:items-end justify-between mb-[43px] gap-4">
            <div><h2 class="text-[32px] font-medium leading-[1.28] tracking-[-0.64px] text-[#1E1D1D] font-['General_Sans',_sans-serif]">Inside <span class="text-[#1447D4]">the UAE:</span> Tips, Insights & Living</h2></div>
            <a href="#" class="h-[52px] inline-flex items-center justify-center px-[32px] py-[16px] border border-[#E8E8E7] rounded-[29.5px] text-[16px] font-medium text-[#1E1D1D] bg-white hover:bg-gray-50 transition tracking-[-0.48px] shadow-sm font-['General_Sans',_sans-serif]">View more articles</a>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-[32px]">
            @foreach([['title'=>'Best rental locations in Dubai for couples','tag'=>'Insights','image'=>'https://images.unsplash.com/photo-1516961642265-531546e84af2?q=80&w=800&auto=format&fit=crop'],['title'=>'What to expect when renting in the UAE for the first time','tag'=>'Community guide','image'=>'https://images.unsplash.com/photo-1518684079-3c830dcef090?q=80&w=800&auto=format&fit=crop'],['title'=>'Hidden Costs to Look Out For When Renting in the UAE','tag'=>'Community guide','image'=>'https://images.unsplash.com/photo-1516961642265-531546e84af2?q=80&w=800&auto=format&fit=crop'],['title'=>'UAE Cultural Norms Every New Resident Should Know','tag'=>'Insights','image'=>'https://images.unsplash.com/photo-1528702748617-c64d49f918af?q=80&w=800&auto=format&fit=crop']] as $article)
                <a href="#" class="group block">
                    <div class="relative overflow-hidden rounded-[6px] aspect-[277/172] mb-4">
                        ...
                        <span class="absolute top-0 right-0 bg-[#1447D4] text-white text-[12px] font-medium px-[13px] py-[4px] rounded-bl-[6px] rounded-tr-[6px]">{{ $article['tag'] }}</span>
                    </div>
                    <h3 class="text-[18px] font-normal text-[#1E1D1D] leading-[1.3] group-hover:text-[#1447D4] transition font-['General_Sans',_sans-serif]">{{ $article['title'] }}</h3>
                </a>
            @endforeach
        </div>
    </div>
...
<div class="bg-white py-[96px]">
    <div class="max-w-[1204px] mx-auto px-4 lg:px-0">
        <div class="flex gap-[80px] items-center">
            <div class="relative rounded-[14px] aspect-square shadow-sm w-[586px] h-[586px]"><img src="{{ asset('images/about_landing_img.png') }}" alt="Cozy Living Room" class="w-full h-full object-cover"></div>
            <div>
                <h2 class="text-[32px] font-medium leading-[1.28] tracking-[-0.64px] text-[#1E1D1D] mb-6 font-['General_Sans',_sans-serif]">About <span class="text-[#1447D4]">HopInHome</span></h2>
                <div class="space-y-6 text-[18px] text-[#464646] leading-[1.5] font-['General_Sans',_sans-serif] w-[474px]">
                    <p>At HopInHome our mission is to make renting <span class="font-medium text-[#1E1D1D]">easier and more transparent</span>. We help renters navigate one of the most stressful parts of moving by providing verified listings, straightforward guidance, and trusted insights from the community.</p>
                    <p>Our goal is to <span class="font-medium text-[#1E1D1D]">reduce surprises</span>, remove uncertainty, and <span class="font-medium text-[#1E1D1D]">help people</span> make confident decisions - without pressure or hidden risks.</p>
                </div>
                <div class="mt-8"><a href="#" class="inline-flex items-center min-w-[160px] h-[52px] justify-center px-[32px] py-[16px] border border-[#1447D4] rounded-[29.5px] text-[16px] font-medium text-[#1447D4] hover:bg-blue-50 transition duration-300 font-['General_Sans',_sans-serif] tracking-[-0.48px]">Learn more</a></div>
            </div>
        </div>
    </div>
</div>
...
<div class="bg-white pb-[128px]">
    <div class="max-w-[1204px] mx-auto px-4 lg:px-0">
        <div class="relative bg-[#1447D4] rounded-[14px] px-[100px] py-[96px] overflow-hidden">
            <div class="absolute top-0 right-0 w-1/2 h-full opacity-10 pointer-events-none"><div class="absolute w-[730px] -bottom-[290px] -right-[110px]"><img src="{{ asset('images/hopinhome_symbol_white.svg') }}" alt="" class="w-full h-auto"></div></div>
            <div class="relative z-10 max-w-[500px]">
                <h2 class="text-[64px] font-medium leading-[1.22] text-[#F9F9F8] mb-6 font-['PP_Formula','General_Sans',_sans-serif] tracking-[-1.92px]">Reach the <br /> Right Renters</h2>
                <p class="text-[#F9F9F8] text-[18px] leading-[1.5] mb-8 font-['General_Sans',_sans-serif]">Publish your listing and connect with people<br>who value clarity and honesty.</p>
                <a href="#" class="inline-flex items-center min-w-[160px] h-[52px] justify-center px-[32px] py-[16px] bg-white text-[#1E1D1D] text-[16px] font-medium rounded-[29.5px] hover:bg-gray-50 transition shadow-sm font-['General_Sans',_sans-serif] tracking-[-0.48px]">Learn more</a>
            </div>
        </div>
    </div>
</div>
